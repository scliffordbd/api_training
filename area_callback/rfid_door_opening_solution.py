...
class RFIDDoorOpeningHandler(AreaCallbackRegionHandlerBase):
    ...
    def run(self):
        ...
        self.safe_sleep(self.settings[PAUSE_KEY])

        # Sample code to handle what to do if the door doesn't open.
        # door_blocked = False  # dummy line so that door is always considered open.
        # if door_blocked:
        #     log_msg = "The robot is unable to cross due to closed door; blocking path."
        #     _LOGGER.error(log_msg)

        #     # Block the path.
        #     raise PathBlocked(log_msg)
